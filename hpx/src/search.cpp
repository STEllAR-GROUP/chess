...
typedef
    actions::plain_result_action4<int, node_t, int, int, int, search>
search_action;

HPX_REGISTER_PLAIN_ACTION(search_action);

typedef lcos::eager_future<search_action> search_future;
...
int think(std::vector<gen_t>& workq, node_t& board, int out)
{
    ...
    for (i = 1; i <= depth[board.side]; ++i) {
        follow_pv = TRUE;
        search_future search(get_runtime().get_process().here(), 
                             board, -10000, 10000, i);
        ...
        if (out == 1)
            ...
            printf("%d %d %d %d",
                    i, val, (timer.elapsed()*1000) / 10, nodes);
        if (out) {
...
int search(node_t board, int alpha, int beta, int depth)
{
    ...
    std::vector< lcos::future_value<int,int> > moves;

    std::vector<gen_t> copy_workq;
    ...
    while (workq.size() != 0) {
        ...
        moves.push_back(search_future(get_runtime().get_process().here(),
                                      board, -beta, -alpha, depth -1));
        copy_workq.push_back(g);

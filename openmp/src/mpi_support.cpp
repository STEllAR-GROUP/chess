...
void *do_mpi_thread(void *voidp) {
    ...
    if(mp->pfunc == search_ab_f) {
        result = search_ab(mp->info->board,mp->info->board.depth,mp->info->alpha,mp->info->beta,mp->info->this_task);
    } else if(mp->pfunc == search_f)
        result = search(mp->info->board,mp->info->board.depth,mp->info->this_task);
    else if(mp->pfunc == qeval_f)
        result = qeval(mp->info->board,mp->info->alpha,mp->info->beta,mp->info->this_task);
